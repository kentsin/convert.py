...
for file in dir_files: # look at every file in the current directory
	if file.endswith('.pdf'): # if it is a PDF, use it
		...
		# 
		# # https://stackoverflow.com/a/50495343
		magick = 'convert -density 150 -crop +55+180 -crop -55-128 "' + file + '.pdf" "' + combined + '-%04d.png"'
		print(magick)
		os.system(magick)
		...
		for pic in pngs:
			if pic.endswith('.png'):
				...
				print(combined_pic)
				tesseract = 'tesseract -l chi_tra+por "' + combined_pic + '" "' + combined_pic + '.txt'
				print(tesseract)
				os.system(tesseract)
